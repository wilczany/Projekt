...
void dodajuczestnika() {
    ...
    printf("Prosze o podanie dokladnych danych. W pozniejszym etapie nie bedzie mozliwo�ci ich edycji.\nPodaj wiek uczestnika.\n");
    scanf("%d", &wiek);

    if (wiek < 18) {
        //ustawianie numer�w i hasla
        u_np.id = 0;
        while (fscanf(plik_np_czytaj, uczestnik_np_format, &u_np.id, &u_np.kod, u_np.daneOsobowe, &u_np.wiek,
                      u_np.email, u_np.opiekun) != EOF) {
            continue;
        }
...
void dodawanieprac() {
    ...
    if (wiek >= 18) {
        plik_czytaj = fopen("uczestnicy.txt", "r");
        if (plik_czytaj == NULL) {
            printf("Blad. Sprobuj ponownie pozniej.\n");
            return;
        }
    }
    ...
    if (wiek < 18) {
        while (fscanf(plik_czytaj, uczestnik_np_format, &u_np.id, &u_np.kod, u_np.daneOsobowe, &u_np.wiek, u_np.email,
                      u_np.opiekun) != EOF) {
